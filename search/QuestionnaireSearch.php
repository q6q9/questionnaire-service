<?php

namespace app\search;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\Questionnaire;
use yii\helpers\ArrayHelper;
use yii\helpers\StringHelper;

/**
 * QuestionnaireSearch represents the model behind the search form of `app\models\Questionnaire`.
 */
class QuestionnaireSearch extends Questionnaire
{
    /**
     * @var string
     */
    public $date_from_to;

    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'is_male', 'rate'], 'integer'],
            [['name', 'email', 'phone', 'region', 'city', 'comment', 'created_at', 'date_from_to'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     * @throws \Exception
     */
    public function search($params)
    {
        $query = Questionnaire::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'is_male' => $this->is_male,
            'rate' => $this->rate,
            'created_at' => $this->created_at,
        ]);

        $query->andFilterWhere(['like', 'name', $this->name])
            ->andFilterWhere(['like', 'email', $this->email])
            ->andFilterWhere(['like', 'phone', $this->phone])
            ->andFilterWhere(['like', 'region', $this->region])
            ->andFilterWhere(['like', 'city', $this->city])
            ->andFilterWhere(['like', 'comment', $this->comment]);

        if ($this->date_from_to) {
            $fromAndTo = StringHelper::explode($this->date_from_to, ' - '); // "yyyy-mm-dd - yyyy-mm-dd"

            $from = ArrayHelper::getValue($fromAndTo, 0);
            $to = ArrayHelper::getValue($fromAndTo, 1);

            $query->andWhere(['>=', 'DATE(created_at)', $from]);
            $query->andWhere(['<=', 'DATE(created_at)', $to]);
        }

        return $dataProvider;
    }
}
